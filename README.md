...
> In VNNCOMP, the meaning of output constraints in the VNN-LIB file are *different* (refer to the ACAS-Xu benchmark in [VNNCOMP'24](https://github.com/VNN-COMP/vnncomp2024_benchmarks)). It defines the "mirrored output constraints" to falsify the property, i.e., if an input satisfies the output constraints, it means that the property is **actually not** satisfied. So if all inputs satisfying the input constraints and do not satisfy the output constraints, it means that the property is **actually satisfied**. There is no differences if we use *complete* approach to verify the output constraints bound because the complete approach provides the *exact* bound with clear SAT/UNSAT. However, if we use some adversarial attack approaches to falsify the property, it will be different, and we need to be aware of this. But why we do this? I think it simplify the original problem. For example, if we want to verify a property consisting many AND clauses, we can falsify the property by checking if one of these clauses is not satisfied. This reduces the high dimensional computation in neural network verification.
